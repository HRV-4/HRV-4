...
class InsertDatabase:
    ...
    def __init__(self, db_user_name, db_user_password, db_port, db_name, HRVvital_analysis, HRVvital_analysis_overview, activity_protocol, HRVmed_analysis, user_folder, raw_excel_path="", processed_excel_path="", txt_path=""):
        """
    ...
    def _read_excel(self, path):
        try:
            ...
            msg = str(e)
            if "io.excel.zip.reader" in msg or (isinstance(path, str) and path.lower().endswith(".zip")):
                try:
                    if isinstance(path, str) and zipfile.is_zipfile(path):
                        with zipfile.ZipFile(path) as zf:
                            excel_names = [
                                n for n in zf.namelist()
                                if n.lower().endswith((".xlsx", ".xlsm", ".xls"))
                            ]
                            if not excel_names:
                                raise
                            with zf.open(excel_names[0]) as f:
                                return pd.read_excel(BytesIO(f.read()), engine="openpyxl")
                except Exception:
                    pass
            raise

    def _is_readable_excel(self, path):
        try:
    ...
    def insert_user(self):
        ...
        try:
            raw_df = self._read_excel(self.raw_excel_path)
            
            # Check if processed Excel exists, if not create it
            if not os.path.exists(self.processed_excel_path):
                print(f"Processed Excel not found: {self.processed_excel_path}")
                print("Creating processed Excel...")
                self.createProcessedExcel()
            
            processed_df = self._read_excel(self.processed_excel_path)
            ...
            user_id = self.user_folder.split("/")[-1]  # adjust according to your filename pattern
    ...
    def insert_raw_data(self):
        ...
        try:
            raw_data_df = self._read_excel(self.raw_excel_path)
            processed_df = self._read_excel(self.processed_excel_path)
            raw_file_name = os.path.basename(self.raw_excel_path)
            raw_file_name = unicodedata.normalize('NFC', raw_file_name)
            user_id = self.user_folder.split("/")[-1]

            measurement_id = None
            if "MeasurementID" in processed_df.columns:
                measurement_id = processed_df.loc[0, "MeasurementID"]
            if pd.isna(measurement_id) or measurement_id in ["", None]:
            ...
            raw_data_df["MeasurementID"] = raw_data_df["MeasurementID"].fillna(measurement_id)
    ...
    def insert_processed_data(self):
        ...
        try:
            processed_data_df = self._read_excel(self.processed_excel_path)
            ...
            required_columns = ['ÖLÇÜM TARİHİ', 'Biyolojik Yaş', 'Biyolojik Yaş (%)', 'Yaş', 'Cinsiyet', 'Heart Beats', 'Min HR', 'Mx HR',
            ...
            for col in required_columns:
                if col not in processed_data_df.columns:
                    processed_data_df[col] = None

            # Clean all object/string columns
            processed_data_df = processed_data_df[required_columns]

            cols = ['Biyolojik Yaş', 'Biyolojik Yaş (%)', 'Yaş', 'Heart Beats', 'Min HR', 'Mx HR',
                    'GVI', 'Dynamic A', 'Dynamic B', 'TP', 'ULF', 'VLF', 'LF', 'HF', 'pNN50', 'SDNN', 'RMSSD ',
                    'TP(Gece)', 'ULF (Gece)', 'VLF (Gece)', 'LF (gece)', 'HF (Gece)', 'pNN50 (Gece)', 'SDNN (Gece)', 'RMSSD (Gece)',
                    'Burnout Resistance (?/10)', 'Burnout Resistance (%)', 'Performance Potential (?/10)', 'Performance Potential (%)',
                    'Processing of Stress (?/10)', 'Processing of Stress (%)', 'State of Health (?/10)', 'State of Health (%)']

            # Handle None values and convert to float64 for numeric columns only
            for col in cols:
                if col in processed_data_df.columns:
                    # Replace string 'None' with actual None/NaN
                    processed_data_df[col] = processed_data_df[col].replace('None', np.nan)
                    # Apply clean_and_float function
                    processed_data_df[col] = processed_data_df[col].map(self.clean_and_float)
                    # Convert any remaining None/NaN to np.nan
                    processed_data_df[col] = processed_data_df[col].fillna(np.nan)
            
            # Convert only numeric columns to float64 (exclude Cinsiyet)
            processed_data_df[cols] = processed_data_df[cols].astype("float64")
            
            # Handle Cinsiyet separately - convert to proper type if needed
            if 'Cinsiyet' in processed_data_df.columns:
                processed_data_df['Cinsiyet'] = processed_data_df['Cinsiyet'].replace('None', None)

            # Convert timestamp to datetime - handle both format variations
            processed_data_df['ÖLÇÜM TARİHİ'] = pd.to_datetime(processed_data_df['ÖLÇÜM TARİHİ'], format='%d.%m.%Y %H:%M', errors='coerce')
            ...
            processed_data_df['start_timestamp'] = pd.to_datetime(processed_data_df['start_timestamp'])
            processed_data_df['end_timestamp'] = pd.to_datetime(processed_data_df['end_timestamp'])

            # Convert measurement_time to timedelta
            computed_measurement_time = processed_data_df['end_timestamp'] - processed_data_df['start_timestamp']
            processed_data_df['measurement_time'] = computed_measurement_time.apply(
                lambda x: None if pd.isna(x) else x.to_pytimedelta()
            ).astype(object)

            # Replace NaT with None for database compatibility
            processed_data_df['start_timestamp'] = processed_data_df['start_timestamp'].where(pd.notna(processed_data_df['start_timestamp']), None)
            processed_data_df['end_timestamp'] = processed_data_df['end_timestamp'].where(pd.notna(processed_data_df['end_timestamp']), None)
            ...
            with self.engine.begin() as conn:
                try:
                    metadata = MetaData()
                    processed_data_table = Table('ProcessedData', metadata, autoload_with=conn)
                    conn.execute(
                        processed_data_table.insert(),
                        processed_data_df.to_dict(orient='records'),
                    )
                except IntegrityError as e:
                    if isinstance(e.orig, psycopg2.errors.UniqueViolation):
    ...
    def createRawExcel(self):
        ...
        if os.path.exists(self.raw_excel_path) and self._is_readable_excel(self.raw_excel_path):
        ...
        if self.HRVvital_analysis is not None:
            create = CreateExcel(
                processed_excel_path=self.processed_excel_path,
            ...
            print("Vital analysis not found.")
            return
        ...
        try:
            ...
            # Fix: timestamps only have 3 decimal places (milliseconds), not 6
            start_timestamp = datetime.strptime(timestamps[0], "%Y-%m-%d %H:%M:%S.%f")
            end_timestamp = datetime.strptime(timestamps[-1], "%Y-%m-%d %H:%M:%S.%f")
            # Calculate measurement_time directly from datetime objects
            measurement_time = end_timestamp - start_timestamp

            measurement_id = str(uuid.uuid4())

            age = vital_analysis["chronological_age_years"]
            gender = vital_analysis["gender"]

            # Update raw Excel template
            raw_df = self._read_excel("raw_template.xlsx")
            ...
            raw_df.loc[0, "Katılımcının:"] = age
            raw_df.loc[1, "Katılımcının:"] = gender
            ...
            # Store timedelta directly (not as string)
            processed_df.loc[0, 'Measurement_Time'] = measurement_time
    ...
    def updateActivities(self):
        ...
        df = self._read_excel(self.raw_excel_path)
        copy_df = self._read_excel(self.raw_excel_path)
    ...
    def clean_and_float(self, s):
        ...
        5. Convert to float and return.
        6. If conversion fails, return np.nan and print the error.
    ...
    def createProcessedExcel(self):
        ...
        """
        # If already created, return
        if os.path.exists(self.processed_excel_path) and self._is_readable_excel(self.processed_excel_path):
        ...
        if os.path.isdir(user_path):
            ...
            for user_date_folder in os.listdir(user_path):
                ...
                if txt_path != "" and HRVvital_analysis != "" and HRVvital_analysis_overview != "" and activity_protocol != "" and HRVmed_analysis != "":
                    ...
                    db = InsertDatabase("postgres", "postgres", "5432", "again", user_folder=user_folder, txt_path=txt_path, raw_excel_path=f"{raw_folder_path}/{filename}_raw.xlsx", processed_excel_path=f"{processed_folder_path}/{filename}.xlsx", 
