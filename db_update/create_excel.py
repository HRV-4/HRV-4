...
class CreateExcel():
    ...
    def parse_vital_analysis(self):
        ...
        # 7) Chronological Age (NOT biological age)
        # Look for specific pattern around "Total average heart rate" 
        m = re.search(r"Total\s+average\s+heart\s+rate.*?(\d+)\s+year\s+old\s+(?:man|woman|male|female)", text_all, self.F)
        if m:
            extracted["chronological_age_years"] = int(m.group(1))
        else:
            # Look for chronological age patterns that are NOT biological age
            m = re.search(r"chronological\s+age[:\s]*(\d+)\s*(?:years?|yaş)", text_all, self.F)
            if m:
                extracted["chronological_age_years"] = int(m.group(1))
            else:
                # Look for age patterns that don't mention "biological"
                m = re.search(r"(?!biological)(?:age|yaş)[:\s]*(\d+)\s*(?:years?|yaş)", text_all, self.F)
                if m:
                    extracted["chronological_age_years"] = int(m.group(1))
            
            # Alternative: Look for patterns like "25 years old" but exclude biological age context
            lines = text_all.split('\n')
            for line in lines:
                if 'biological' not in line.lower():
                    m = re.search(r"(\d+)\s*(?:years?|yaş)\s*old", line, self.F)
                    if m:
                        extracted["chronological_age_years"] = int(m.group(1))
                        break
        ...
        if m:
            ...
            m = re.search(r"(?:gender|sex)[:\s]*(male|female|erkek|kadın|bay|bayan)", text_all, self.F)
            if m:
                gender = m.group(1).lower()
                # Normalize gender values
                if gender in ["erkek", "bay"]:
                    gender = 1
                elif gender in ["kadın", "bayan"]:
                    gender = 2
                extracted["gender"] = gender
            
            # Alternative gender patterns
            m = re.search(r"\b(male|female|erkek|kadın|bay|bayan)\b", text_all, self.F)
            if m:
                gender = m.group(1).lower()
                if gender in ["erkek", "bay"]:
                    gender = 1
